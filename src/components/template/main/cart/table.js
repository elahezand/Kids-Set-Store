...
const Table = () => {
  ...
  const { handleSubmit, control, formState: { errors } } = useForm({
    ...
    defaultValues: { fullName: "", phone: "", address: "", city: "", postalCode: "" },
  });

  const submitOrder = (data) => {
    if (!cart.length) return toast.error("Cart is empty");
    createOrder({ items: cart, totalPrice: discountedTotal, shippingAddress: data });
  };
  ...
  return (
    <>
        ...
        {cart.length ? (
          <>
            <div className="w-full overflow-x-auto rounded-2xl shadow-card">
              <table className="w-full min-w-[640px] border-collapse bg-white dark:bg-gray-800 text-text dark:text-gray-100">
                <thead>
                  <tr>
                    <th className="bg-sage-400 p-4 text-center text-sm font-semibold uppercase tracking-wide text-white">Product</th>
                    <th className="bg-sage-400 p-4 text-center text-sm font-semibold uppercase tracking-wide text-white">Price</th>
                    <th className="bg-sage-400 p-4 text-center text-sm font-semibold uppercase tracking-wide text-white">Number</th>
                    <th className="bg-sage-400 p-4 text-center text-sm font-semibold uppercase tracking-wide text-white">Total</th>
                    ...
                    <tr>
                      <td className="w-[250px] border-b border-gray-200 p-4 text-left align-middle dark:border-white/10">
                        <div className="flex items-center gap-4">
                          <Image width={80} height={80} src={item.img} alt="" className="h-20 w-20 rounded-xl object-cover" />
                          <Link href="/" className="text-sm font-medium leading-6 text-text dark:text-gray-100">{item.name}</Link>
                        </div>
                      </td>
                      ...
                      <td className="min-w-[180px] border-b border-gray-200 p-4 text-center align-middle dark:border-white/10">
                        <div className="mx-auto flex w-[100px] items-center justify-between overflow-hidden rounded-lg border-2 border-coral-300">
                          <span onClick={() => decreaseCount(item._id)} className="flex-1 cursor-pointer select-none bg-gray-50 dark:bg-gray-800 py-1 text-center transition-colors hover:bg-coral-300 hover:text-white"> - </span>
                          {item.count}
                          <span onClick={() => increaseCount(item._id)} className="flex-1 cursor-pointer select-none bg-gray-50 dark:bg-gray-800 py-1 text-center transition-colors hover:bg-coral-300 hover:text-white"> + </span>
                        </div>
                      </td>
                      ...
                      <td className="border-b border-gray-200 p-4 text-center align-middle dark:border-white/10" onClick={() => removeFromCart(item._id)}>
          ...
          <div className="card card-body py-16 text-center">
            ...
            <p className="mb-4 text-2xl font-bold sm:text-[32px]">No Product yet</p>
            <div>
      ...
      <div className="card card-body w-full lg:w-[380px] lg:shrink-0">
        <p className="mb-4 text-lg font-bold uppercase sm:text-xl">Cart Total</p>
        <form onSubmit={handleSubmit(submitOrder)} className="flex w-full flex-col gap-3.5">
          ...
          <div className="mt-2 flex flex-row items-center justify-between border-t border-gray-200 pt-4 dark:border-white/10">
            <p>Total</p>
            <p className="text-xl font-bold text-sage-500">{discountedTotal.toLocaleString()} $</p>
          </div>
